...
<body>
    ...
    <div class="data-emissao">
        <span class="label">Inventário de Estoque</span>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="text-right" style="width: 120px;">Preço Custo</th>
        ...
        <tbody>
            ...
            @foreach($dados as $item)
                @php($quantidade = $item->estoque->quantidade ?? 0)
                @php($valor = $quantidade * $item->preco_custo)
                @php($soma += $valor)
                @php($totalItens += $quantidade)
                <tr>
                    <td>{{ $item->descricao }}</td>
                    <td>{{ $item->tamanho }}</td>
                    <td class="text-right">R$ {{ number_format($item->preco_custo, 2, ',', '.') }}</td>
        ...
        <tfoot>
            <tr>
                <td colspan="4" class="text-right">TOTAL EM ESTOQUE:</td>
    ...
    <div class="totals">
        <p>Total de Produtos: <strong>{{ $dados->count() }}</strong> produto(s)</p>
        <p>Quantidade Total em Estoque: <strong>{{ $totalItens }}</strong> unidade(s)</p>
        <p>Valor Total Investido: <strong>R$ {{ number_format($soma, 2, ',', '.') }}</strong></p>
    </div>
</body>
</html>
